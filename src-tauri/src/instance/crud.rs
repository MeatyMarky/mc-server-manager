...
pub async fn create(state: &AppState, input: CreateInstanceInput) -> AppResult<Instance> {
    ...
    // Only where this server type can actually run it: a request that arrived
    // for the wrong type is dropped rather than installed and left to fail.
    let map_kind = (input.web_map && crate::map::supported(input.server_type))
        .then(|| crate::map::SLUG.to_string());
